...
class SqliteDriver(object):
    ...
    def load(self):
        self._cursor.execute(
            "CREATE TABLE employee (name varchar(255), dept char(1), birth int, salary double);"
        )
    ...
    def groupby(self):
        self._cursor.execute(
            "SELECT avg(birth), sum(salary) FROM employee GROUP BY dept;"
        )
        self._conn.commit()
    ...
    def join(self):
        self._cursor.execute(
            "SELECT employee.name, employee.salary + bonus.bonus "
